using UnityEngine;
using System.Collections;
using System.Collections.Generic;
namespace Assets.Model.Ship{
    public class ShipCore {

        public List<ShipComponent> Components { get; private set; }
        public int MaxHull { get; private set; }
        public int CurrentHull { get; set; }
        public int MaxHeat { get; private set; }
        public int CurrentHeat { get; set; }
        public int MaxPower { get; private set; }
        public int CurrentPower { get; private set; }
        public int MaxAmmo { get; private set; }
        public int CurrentAmmo { get; private set; }

        public float MaxSpeed { get; private set; }
        public float CurrentSpeed { get; set; }

        public float MaxTurningSpeed { get; private set; }
        public float CurrentTurningSpeed { get;  set; }

		public float MaxAcceleration { get; private set; }
		public float CurrentAcceleration{ get; set; }

		public float MaxTurningAcceleration{ get; private set; }
		public float CurrentTurningAcceleration{ get; set; } 

		public ShipCore(int maxHull,int maxPower, int maxHeat, int maxAmmo, float maxSpeed, float maxTurningSpeed, float maxAcceleration, float maxTurningAcceleration, List<ShipComponent> components)
        {

            this.MaxHull = maxHull;
            this.CurrentHull = maxHull;
            this.MaxHeat = maxHeat;
            this.CurrentHeat = 0;
            this.MaxPower = maxPower;
            this.CurrentPower = maxPower;
            this.MaxAmmo = maxAmmo;
            this.CurrentAmmo = maxAmmo;
            this.MaxSpeed = maxSpeed;
            this.CurrentSpeed = 0;
            this.MaxTurningSpeed = maxTurningSpeed;
            this.CurrentTurningSpeed = 0;
			this.MaxAcceleration = maxAcceleration;
			this.CurrentAcceleration = 0;
			this.MaxTurningAcceleration = maxTurningAcceleration;
			this.CurrentTurningAcceleration = 0;
            this.Components = components;
        }

		public ShipCore(ShipCore toCopy):this(toCopy.MaxHull,toCopy.MaxPower, toCopy.MaxHeat, toCopy.MaxAmmo,  toCopy.MaxSpeed, toCopy.MaxTurningSpeed, toCopy.MaxAcceleration, toCopy.MaxTurningAcceleration,toCopy.Components)
        {
           
        }

        public override string ToString()
        {
            string shipAsString = string.Format("Ship, MHu:{0}, MP:{1}, MHe:{2}, MA:{3}, MS: {4}, MAS: {5}\n", MaxHull, MaxPower, MaxHeat,MaxAmmo,MaxSpeed,MaxTurningSpeed);
            shipAsString += "Componenents:\n";
            foreach(ShipComponent comp in this.Components)
            {
                shipAsString += comp + "\n";
            }
            return shipAsString;
        }

    }
}
